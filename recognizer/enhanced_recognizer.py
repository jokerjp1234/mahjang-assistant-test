...
from recognizer.meld_recognizer import MeldRecognizer

logger = logging.getLogger("MahjongAssistant.Recognizer")


class EnhancedMahjongRecognizer:
    """雀魂用拡張牌認識クラス（副露対応）"""
    ...
    def __init__(self, screen_areas=None):
        ...
        # 副露認識クラス
        self.meld_recognizer = MeldRecognizer(model_dir=model_path)
        ...
        logger.info("EnhancedMahjongRecognizer初期化完了")
    ...
    def detect_game_state(self):
        ...
        # 副露の検出
        self.player_melds = self.meld_recognizer.detect_melds(screen)
        
        # 自分の副露数
        own_meld_count = len(self.player_melds[0])
        
        # 副露数に応じて手牌エリアとツモ牌エリアを調整
        hand_img, draw_tile_img = self.meld_recognizer.adjust_hand_area(screen, own_meld_count)
        ...
        # ツモ牌を保存
        self.draw_tile = draw_tile_img
        
        return {
            'hand_img': hand_img,
            'draw_tile_img': draw_tile_img,
            'dora_img': dora_img,
            'river_imgs': river_imgs,
            'reach_indicators': reach_indicators,
            'scores': scores,
            'current_player': current_player,
            'player_melds': self.player_melds
        }
    ...
    def recognize_hand_tiles(self, hand_img):
        ...
        if self.model is None:
            ...
            if self.last_demo_data is None or current_time - self.last_update_time > 30:
                # 副露数に応じて手牌枚数を調整
                own_meld_count = len(self.player_melds[0])
                hand_size = 13 - (own_meld_count * 3)  # 副露1セットにつき3枚減る
                if hand_size < 1:
                    hand_size = 1  # 最低でも1枚は残す
                
                self.last_demo_data = self._generate_demo_hand_tiles(hand_size)
                self.last_update_time = current_time
                logger.debug(f"デモ手牌データを更新しました（{hand_size}枚）")
    ...
    def recognize_draw_tile(self):
        ...
        -------
        int or None
            認識されたツモ牌のID、または None
        """
        if self.draw_tile is None:
            return None
        
        if self.model is None:
            # デモモード: ランダムなツモ牌を生成
            all_tiles = list(range(0, 136, 4))  # 0, 4, 8, ...
            # 手牌に含まれていない牌からランダムに選ぶ
            ...
            if all_tiles:
                ...
                return 0  # 候補がない場合は1萬とする
        
        # モデルがある場合の処理
        # ツモ牌画像から牌を識別
        return self._identify_tile(self.draw_tile)
    ...
    def recognize_meld_tiles(self, player_idx=0):
        """
        指定プレイヤーの副露牌を認識
        ...
        list
            認識された副露牌のリスト
        """
        if player_idx < 0 or player_idx >= len(self.player_melds):
            return []
        
        # 副露情報を牌リストに変換
        return self.meld_recognizer.decode_melds_to_tiles(self.player_melds[player_idx])
    ...
    def get_all_visible_tiles(self):
        ...
        """
        # 手牌
        hand_tiles = self.recognize_hand_tiles(None)  # 既に保存された状態を使用
        
        # ツモ牌
        draw_tile = self.recognize_draw_tile()
        
        # 自分の副露牌
        meld_tiles = self.recognize_meld_tiles(0)
        
        # 相手の副露牌
        opponent_meld_tiles = []
        for idx in range(1, 4):
            opponent_meld_tiles.extend(self.recognize_meld_tiles(idx))
        
        # 河の牌
        river_tiles = []
        for idx in range(4):
            river_tiles.extend(self.recognize_river_tiles(None))  # ダミー引数
        
        # ドラ表示牌
        dora_indicators = self.recognize_dora_indicators(None)  # ダミー引数
        ...
        all_tiles.extend(opponent_meld_tiles)
        all_tiles.extend(river_tiles)
        all_tiles.extend(dora_indicators)
        
        # 牌種別ごとにカウント
        tile_counts = {}
        for tile in all_tiles:
            tile_type = tile // 4  # 4で割って牌種別に変換
    ...
    def _empty_game_state(self):
        ...
        return {
            ...
            'draw_tile_img': np.zeros((70, 40, 3), dtype=np.uint8),
    ...
    def _generate_demo_hand_tiles(self, hand_size=13):
        """
        デモ用の手牌データを生成（より麻雀らしい手牌）
        
        Parameters
        ----------
        hand_size : int, optional
            手牌のサイズ（デフォルト: 13）
        ...
        if hand_size <= 4:
            # シンプルにランダムな牌を選択
            all_tiles = [i * 4 for i in range(34)]  # 34種類の牌
            return random.sample(all_tiles, hand_size)
        
        # 一定確率でメンツ（同じ数字3枚）を含める
        max_mentsu = min(hand_size // 3, 2)  # 手牌サイズに応じてメンツの最大数を制限
        mentsu_count = random.randint(0, max_mentsu)
        
        for _ in range(mentsu_count):
            # メンツの基本牌を選ぶ
            base_tile = random.randint(0, 33)
            tiles.extend([base_tile * 4, base_tile * 4 + 1, base_tile * 4 + 2])
        
        # 一定確率で順子（連続する3枚）を含める
        if hand_size - len(tiles) >= 3:
            max_shuntsu = min((hand_size - len(tiles)) // 3, 2)  # 残りの手牌サイズに応じて順子の最大数を制限
            shuntsu_count = random.randint(0, max_shuntsu)
            
            for _ in range(shuntsu_count):
                # 数牌から選ぶ（1-7スタート）
        ...
        logger.debug(f"デモ手牌生成: {tiles} ({hand_size}枚)")
        return tiles
